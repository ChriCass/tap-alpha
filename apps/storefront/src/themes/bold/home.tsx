    ...
    <div className="border-b border-white/10">
      <div className="mx-auto max-w-7xl px-6 py-8">
        <input
          ...
          className="w-full max-w-md rounded-(--theme-radius) border border-white/15 bg-white/5 px-5 py-3.5 text-sm font-medium uppercase tracking-wider text-white outline-none transition-colors placeholder:text-neutral-500 focus:border-white/40"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16">
      {loading ? (
        ...
        <p className="text-sm uppercase tracking-widest text-neutral-500">Sin resultados</p>
      ) : (
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => (
            <Link
              ...
              className="group overflow-hidden rounded-(--theme-radius) border border-white/10 bg-neutral-900 transition-colors hover:border-white/30"
            >
              ...
              <div className="flex items-start justify-between gap-4 p-5">
                ...
                <span
                  className="shrink-0 rounded-(--theme-radius) px-3 py-1.5 text-sm font-black text-neutral-950"
                  style={{ background: "var(--theme-accent)" }}
                >
...
export function BoldHome(props: ThemeHomeProps) {
