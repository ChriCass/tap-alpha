...
export function ProductDetailPage() {
  ...
  if (product === null) {
    return <p className="mx-auto max-w-6xl px-4 py-16 text-sm text-gray-500">Cargando…</p>;
  }

  if (product === "not-found") {
    return (
      <div className="mx-auto max-w-6xl px-4 py-16 text-center">
        <p className="text-sm text-gray-500">No encontramos este producto.</p>
        <Link to="/" className="mt-3 inline-block text-sm text-gray-900 underline">
          Volver al catálogo
        </Link>
      </div>
    );
      ...
      <div className="mt-6 grid grid-cols-1 gap-10 md:grid-cols-2">
        ...
        <div>
          ...
          {product.is_personalizable && (
            <span className="mt-3 inline-block rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700">
              Personalizable
            </span>
          )}
